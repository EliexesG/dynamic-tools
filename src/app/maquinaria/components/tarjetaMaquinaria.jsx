...
export default function TarjetaMaquina({ titulo, descripcion, imagenes }) {
  ...
  return (
    <article className="row p-3 mb-4 rounded border border-1 border-primary border-start">
      <div
        className={`col-md-6 ${
          mostrarImagenesCompleto ? "mostrarImagenesCompleto" : ""
        }`}
      >
        <div className="pt-2">
          <button
        ...
        <div className="border-top border-primary pt-3 pb-3 row align-items-center justify-content-center contenedorImagenes mt-3">
          {imagenes.map((imagen) => (
            <Imagen
              key={imagen.id}
              className={"col-sm-6 imagenMaquina rounded p-0 m-2 shadow"}
              alt={`imagen ${titulo} ${imagen.id}`}
              src={imagen.url}
              width={1600}
              height={1201}
            />
      ...
      <div
        className="col-md-6 d-flex align-items-center justify-content-center flex-column mt-4"
        id="contenedorCartaTexto"
      >
        <h2 className="fw-bold text-center rounded p-2 bg-primary text-white fw-bold fs-4">
          {titulo}
        </h2>
        <p>{descripcion}</p>
      </div>
